...
<svelte:head>
	...
	<meta property="og:url" content="https://stevetom.vercel.app/projects" />
	...
	<meta name="twitter:url" content="https://stevetom.vercel.app/projects" />
	...
	<main class="hero-section mx-auto min-h-screen pb-20">
		...
		<div class="max-w-7xl mx-auto px-4 py-16 md:py-24">
			...
			<div class="text-center mb-16" in:fly={{ y: 30, duration: 800, easing: cubicInOut }}>
				<div
					class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/20 text-white/80 mb-6"
				>
					<Star size="16" class="animate-pulse" />
					<span class="text-sm font-semibold uppercase tracking-wider">Portfolio</span>
				</div>
				<h1 class="text-5xl md:text-7xl font-bold mb-6 text-white">
					Featured <span class="text-transparent [-webkit-text-stroke:1px_rgba(255,255,255,0.8)]"
						>Projects</span
					>
				</h1>
			...
			{#if loading}
				<div class="flex flex-col items-center justify-center py-20" in:fade>
					<div
						class="w-16 h-16 border-4 border-white/10 border-t-white rounded-full animate-spin"
					></div>
					...
					{#each projects as project, index (project.id)}
						...
						>
							...
							<div class="relative overflow-hidden aspect-video">
								...
								<div
									class="absolute inset-0 flex items-center justify-center gap-3 opacity-0 group-hover:opacity-100 transition-all duration-500"
								>
									<a
								...
								<div class="absolute top-4 left-4 flex items-center gap-2">
									<span
										class="px-3 py-1.5 text-xs font-bold rounded-full border backdrop-blur-md {getStatusColor(
											project.status
							...
							<div class="p-6">
								<h2
									class="text-2xl font-bold mb-3 text-white group-hover:text-white/90 transition-colors"
								>
									{project.title}
								</h2>
								...
								<div class="flex flex-wrap gap-2">
									{#each project.tech.slice(0, 5) as tech}
										<div class="group/tech relative">
											<div
												class="flex items-center gap-2 px-3 py-2 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 hover:border-white/30 transition-all duration-300"
											>
									...
									{#if project.tech.length > 5}
										<div
											class="flex items-center px-3 py-2 bg-white/5 border border-white/10 rounded-xl text-xs text-white/50 font-medium"
										>
							...
							<div
								class="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-transparent via-white/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"
							></div>
						</article>
					{/each}
				</div>
